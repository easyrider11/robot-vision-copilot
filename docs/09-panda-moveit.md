...
零指令下仍漂移 —— 当时据此推断"问题不在我们的命令链"。（这个推断后来
只被证实了一半：命令链确实没错，但锅也不在 servo，见下方更正。）与其调一个黑盒，不如用 40 行可单测的
...
> 完整复现材料（Dockerfile、launch、三个探针脚本、原始数据）：
> [docs/assets/servo-repro/](assets/servo-repro/RESULT.md)。
